function PlayerStandard:_update_fwd_ray()
	...
	managers.environment_controller:set_dof_distance(math.max(0, math.min(self._fwd_ray and self._fwd_ray.distance or 4000, 4000) - 200), (self._state_data.in_steelsight or self:_is_using_bipod()))

	if alive(self._equipped_unit) then
		if (self._state_data.in_steelsight or self:_is_using_bipod()) and self._fwd_ray and self._fwd_ray.unit and self._equipped_unit:base().check_highlight_unit then
